      ...
      <tr id="filesarchive<?=$d->archive_id?>">
        ...
        <td>
          <div class="hidden-sm hidden-xs btn-group">
            <a href="<?php echo base_url();?>User/download_one_file_archive/<?=$d->archive_id?>" class="tooltip-info" data-rel="tooltip" title="Download">
          ...
          <div class="hidden-md hidden-lg">
            <div class="inline pos-rel">
              ...
              <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                <li>
                  <a href="<?php echo base_url();?>User/download_one_file_archive/<?=$d->archive_id?>" class="tooltip-info" data-rel="tooltip" title="View">
                    <span class="blue">
                      <i class="ace-icon fa fa-download bigger-120"></i>
                    </span>
                  </a>
		...
		swal({
			...
			function(isConfirm) {
				if (isConfirm) {
					$.ajax({
						type: 'POST',
						url: "<?php echo base_url();?>User/delete_one_file_archive/" + id,
...
<script type="text/javascript">
	$(document).ready(function() {
      ...
      $('#loadingdiv').fadeOut('fast', function() {
        $('#loadingdiv').empty();
        $("#loadingdiv").load('<?php echo base_url().'User/load_files/'; ?>' + folderid+"/"+departmentid,
        function() {
          $('#spinner').hide();
