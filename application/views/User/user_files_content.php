<h3 class="header smaller lighter blue"><?=$foldername?></h3>
      ...
      <tr id="filesedit<?=$d->files_id?>">
        ...
        <td>
          ...
          <?php foreach($users as $u):?>
            <?php if($d->files_user_id_fk == $u->user_id):?>
              ...
              <p class="help-block">Uploaded by:<?=$name?></p>
            <?php endif;?>
          <?php endforeach;?>
          <?php else:?>
          <?php foreach($users as $u):?>
        ...
        <td>
          <div class="hidden-sm hidden-xs btn-group">
            <a href="<?php echo base_url();?>User/download_one_file/<?=$d->files_id?>" class="tooltip-info" data-rel="tooltip" title="Download">
            ...
            <a href="<?php echo base_url();?>User/view_one_file/<?=$d->files_id?>" class="tooltip-info" data-rel="tooltip" title="View">
          ...
          <div class="hidden-md hidden-lg">
            <div class="inline pos-rel">
              ...
              <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                <li>
                  <a href="<?php echo base_url();?>User/download_one_file/<?=$d->files_id?>" class="tooltip-info" data-rel="tooltip" title="View">
                    <span class="blue">
                      <i class="ace-icon fa fa-download bigger-120"></i>
                    </span>
                  </a>
                ...
                <li>
                  <a href="<?php echo base_url();?>User/view_one_file/<?=$d->files_id?>" class="tooltip-success" data-rel="tooltip" title="View">
                    <span class="light">
                      <i class="ace-icon fa fa-eye bigger-120"></i>
                    </span>
                  </a>
		...
		swal({
			...
			function(isConfirm) {
				if (isConfirm) {
					$.ajax({
						type: 'POST',
						url: "<?php echo base_url();?>User/delete_one_file/" + id,
...
<script type="text/javascript">
	$(document).ready(function() {
      ...
      $('#loadingdiv').fadeOut('fast', function() {
        $('#loadingdiv').empty();
        $("#loadingdiv").load('<?php echo base_url().'User/load_archive/'; ?>' + id + "/" + departmentid + "/" + folderid,
        function() {
          $('#spinner').hide();
